...
// 定义接收流事件的样例类。其是第三方支付平台给我们返回的支付日志信息
...
object TxMacthDetect {
  ...
  class TxPayMatch() extends CoProcessFunction[OrderEvent, ReceiptEvent, (OrderEvent, ReceiptEvent)] {
    ...
    // 订单支付事件数据的处理
    // 因为两个流都提前按照tx_id做了分组，所以下面两个流处理的是同一个tx_id对应的event
    override def processElement1(pay: OrderEvent, ctx: CoProcessFunction[OrderEvent, ReceiptEvent, (OrderEvent, ReceiptEvent)]#Context, out: Collector[(OrderEvent, ReceiptEvent)]): Unit = {
      ...
      if (receipt != null) {
        ...
        receiptState.clear()
        // 个人觉得貌似在匹配上之后则应该删除定时器，否则定时器一定会执行
      } else {
        // 如果还没到，那么把pay存入状态，并且注册一个定时器等待
        // 此处的情况为订单流里面的支付event已经得到了，但是到账的event还没有到达。
        // 但是到账的event还没到达的原因可能是延时5s导致的
        // 所以此时定义一个5s之后再触发的定时器，再次去判断，到账的event是否已经到达
        // 如果5s之后到账的event还没有达到，则报警
        payState.update(pay)
