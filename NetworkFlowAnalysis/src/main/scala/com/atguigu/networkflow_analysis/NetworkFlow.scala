...
object NetworkFlow {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dataStream = env.socketTextStream("localhost", 7777)
      ...
      }) //延时时间，可以先大概的观测日志给出一个值，然后则应该测试日志信息，一般延时时间不应该超出30s。因为此处为每隔5s统计一次结果，所以延时的时间应该在5s左右比较合理
      .assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor[ApacheLogEvent](Time.seconds(1)) {
        override def extractTimestamp(element: ApacheLogEvent): Long = element.eventTime
      })
      .keyBy(_.url) // 统计谁，就按照谁进行keyBy
...
//  input为预聚和函数CountAgg的输出结果。此处CountAgg对于一个窗口里面的所有数据进行聚合，最后只有一个输出的结果
...
class TopNHotUrls(topSize: Int) extends KeyedProcessFunction[Long, UrlViewCount, String] {
  ...
  override def onTimer(timestamp: Long, ctx: KeyedProcessFunction[Long, UrlViewCount, String]#OnTimerContext, out: Collector[String]): Unit = {
    ...
    val sortedUrlViews: mutable.Seq[(String, Long)] = allUrlViews.sortWith(_._2 > _._2).take(topSize)
